...
from training.train import Trainer, DescriptorScaler
from models.cvae import sequence_to_indices, indices_to_sequence
from descriptors.descriptor_calculator import (
...
def modify_descriptor_to_target(orig_descriptor: np.ndarray,
    ...
    if use_default_target:
        ...
        # GRAVY 拉到 (-0.3, 0.4)
        gravy = des[name_to_idx["GRAVY"]]
        if gravy > 0.5:
            _set("GRAVY", 0.3)
        elif gravy < -0.5:
            _set("GRAVY", -0.2)
        # 疏水比例
        _set("hydrophobic_ratio", max(0.35, min(0.5, des[name_to_idx["hydrophobic_ratio"]])))
        # 芳香比例适度
        arom = des[name_to_idx["aromatic_ratio"]]
        if arom > 0.20:
            _set("aromatic_ratio", 0.15)
        # 降低溶血、毒性、聚集
        _set("hemolysis_risk", 0.15)
        _set("toxicity_risk", 0.15)
        _set("aggregation_propensity", 0.15)
        # instability 调低
    ...
    for seq in sequences:
        ...
        for strat in strategies:
            for sp in range(n_samples_per_seq):
                masked_seq, mask_indices = apply_mask(seq, strat, mask_ratio=mask_ratio,
                                                      seed=seed + sp + hash(strat) % 1000)
                # 构造 batch
                ...
                with torch.no_grad():
                    gen_idx, z = trainer.model.reconstruct_or_generate(
                        src_t, cond_t, max_len=MAX_SEQ_LEN + 2,
                        temperature=temperature, top_k=trainer.cfg.top_k, sample_z=True
                    )
                    gen_seq = indices_to_sequence(gen_idx[0].cpu().tolist())
...
def generate_from_lead_peptides(trainer: Trainer,
    ...
    for lead, lead_id in zip(leads, lead_ids):
        ...
        target_des_arr = modify_descriptor_to_target(orig_des_arr)
            ...
            for temp in temperatures:
                ...
                for k in range(per_temp):
                    with torch.no_grad():
                        gen_idx, z = trainer.model.reconstruct_or_generate(
                            src_t, cond_t, max_len=MAX_SEQ_LEN + 2,
                            temperature=temp, top_k=trainer.cfg.top_k, sample_z=True
                        )
                    gen_seq = indices_to_sequence(gen_idx[0].cpu().tolist())
                    # 过滤无效
                    ...
                    rec = {
                        ...
                        "candidate_id": f"{lead_id}_{mask_tag}_t{temp:.1f}_{k:02d}",
                    }
                    # 加入描述符
...
def run_generation_pipeline(trainer: Trainer = None) -> pd.DataFrame:
    ...
    if not candidates_df.empty:
        ...
        candidates_df = candidates_df.drop_duplicates(subset=["generated_sequence"]).reset_index(drop=True)
        print(f"[GEN] 去重: {n_before} → {len(candidates_df)}")
