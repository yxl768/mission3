...
def get_cand_col(df, logical_name):
    ...
    return df[col] if col in df.columns else df[logical_name]
...
def validate_candidates_properties():
    ...
    for p in props_logical:
        cand_col = CANDIDATE_COL_MAP.get(p, p)
        train_col = p
        if cand_col not in cand.columns:
            continue
        cand_vals = cand[cand_col].dropna()
...
def validate_candidates_motif():
    ...
    cand_path = os.path.join(RESULTS_DIR, "generated_candidates.csv")
    cand = pd.read_csv(cand_path)
    seq_col = CANDIDATE_COL_MAP.get("sequence", "sequence")
    seqs = cand[seq_col].tolist()
...
def validate_top_candidates_detail():
    ...
    # 按溶血风险升序排列，取最优
    hemo_col = CANDIDATE_COL_MAP.get("hemolysis_risk", "hemolysis_risk")
    act_col = CANDIDATE_COL_MAP.get("predicted_activity", "predicted_activity")
    charge_col = CANDIDATE_COL_MAP.get("net_charge", "net_charge")
    gravy_col = CANDIDATE_COL_MAP.get("GRAVY", "GRAVY")
    krh_col = CANDIDATE_COL_MAP.get("KRH_ratio", "KRH_ratio")
    tox_col = CANDIDATE_COL_MAP.get("toxicity_risk", "toxicity_risk")
    seq_col = CANDIDATE_COL_MAP.get("sequence", "sequence")
...
def validate_novelty_analysis():
    ...
    cand = pd.read_csv(cand_path)
    seq_col = CANDIDATE_COL_MAP.get("sequence", "sequence")
    cand_seqs = set(cand[seq_col].unique())
    ...
    print(f"\n  序列长度分布:")
    lengths = cand[CANDIDATE_COL_MAP.get("length", "length")]
