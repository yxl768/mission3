...
def sequence_to_indices(seq: str, max_len: int = MAX_SEQ_LEN,
    ...
    # Masked strings are represented as e.g. ``KL[MASK]LL``.  Splitting
    # character-by-character would expand one residue position into six
    # unknown tokens and corrupt both length and mask alignment.
    i = 0
    special = sorted((tok for tok in AA_TO_IDX if tok.startswith("[")),
                     key=len, reverse=True)
    while i < len(seq):
        token = next((tok for tok in special if seq.startswith(tok, i)), None)
        if token is not None:
            indices.append(AA_TO_IDX[token])
            i += len(token)
        else:
            indices.append(AA_TO_IDX.get(seq[i], AA_TO_IDX["[UNK]"]))
            i += 1
    if add_sos_eos:
...
class ConditionalVAE(nn.Module):
    ...
    def __init__(self, cfg: ModelConfig):
        ...
        # Direct masked-token head.  The autoregressive decoder is retained
        # for the CVAE objective, while this head makes local infilling an
        # explicit supervised task instead of forcing all information through
        # a single sampled latent vector.
        enc_dim = cfg.encoder_hidden * 2
        self.mask_head = nn.Sequential(
            nn.Linear(enc_dim + cfg.descriptor_dim, cfg.decoder_hidden // 2),
            nn.LayerNorm(cfg.decoder_hidden // 2),
            nn.GELU(),
    ...
    def forward(self, src_idx: torch.Tensor, tgt_idx: torch.Tensor,
        ...
        返回：
          ...
          latent_z: [B,Z]
        """
        mu, logvar, enc_out, _ = self.encoder(src_idx, cond, src_len)
    ...
    def compute_loss(self, logits: torch.Tensor, tgt_idx: torch.Tensor,
                     ...
                     infill_logits: torch.Tensor = None,
                     src_masked_flags: torch.Tensor = None,
                     infill_weight: float = 1.5):
        """
        损失 = 重建交叉熵 + KL散度
        ...
        infill_loss = torch.zeros((), device=logits.device)
        if infill_logits is not None and src_masked_flags is not None:
            # src positions are [SOS, residues..., EOS, PAD...], so the
            # target sequence is aligned after the same SOS offset.
            labels = tgt_idx
            ce = F.cross_entropy(
                infill_logits.reshape(-1, V), labels.reshape(-1),
                ignore_index=PAD_IDX, reduction="none"
            ).view(B, -1)
            flags = src_masked_flags.float()
            valid = flags * (labels != PAD_IDX).float()
            infill_loss = (ce * valid).sum() / (valid.sum() + 1e-8)

        total_loss = recon_loss + infill_weight * infill_loss + kl_weight * kl_loss
        return {
            "total": total_loss,
            "recon": recon_loss,
            "infill": infill_loss,
    ...
    @torch.no_grad()
    def predict_masked_logits(self, src_idx: torch.Tensor, cond: torch.Tensor,
                              src_len: torch.Tensor = None) -> torch.Tensor:
        """Return per-position logits for deterministic/stochastic infilling."""
        self.eval()
        _, _, enc_out, _ = self.encoder(src_idx, cond, src_len)
        cond_b = cond.unsqueeze(1).expand(-1, enc_out.shape[1], -1)
    ...
    def reconstruct_or_generate(self, src_idx: torch.Tensor, cond: torch.Tensor,
        ...
        for step in range(max_len - 1):
            ...
            # top-k sampling; top_k==1 is deterministic greedy decoding.
            if top_k == 1:
                next_token = logits.argmax(dim=-1)
            elif top_k > 1 and top_k < logits.shape[-1]:
...
if __name__ == "__main__":
    ...
    logits, mu, logvar, z, infill_logits = model(src, tgt, cond)
